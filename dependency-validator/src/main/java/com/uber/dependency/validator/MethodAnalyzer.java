...
public class MethodAnalyzer {
  ...
  /**
   * If we want to share the traversal tracker across different invocations, we need to make sure that
   ...
   * site was analyzed.
   *
   * <p>As an example, consider a call to method foo(). The first time
   ...
   *
   * <p>Now consider that before analyzing another jar, class AAA was removed from the
   * "allocation set". Then, when analyzing this new jar, a call to method foo() is encountered
   ...
   * jars may lead to false positives. In the example above, we will keep both AAA and ZZZ
   * classes when analyzing the second jar even though AAA may have never be encountered when
   * analyzing this jar separately.
   */
   ...
   * @param jar where the missing method is searched
   ...
   *
   * @param numFound number of missing methods found so far
   * @param name of the jar being analyzed
   ...
   * @param analysisStartTime time the entire analysis for this jar has started
   * @return <code>true</code> number of missing methods found, inluding
   ...
   * Since the analysis works across multiple jars, a given method reference may represent more
   * then one actual method as separate jar may use the same class/method declaration but a
