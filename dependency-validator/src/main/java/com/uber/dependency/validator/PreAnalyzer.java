...
public class PreAnalyzer {
   ...
   */
  public PreAnalyzer(
      List<String> rootJarsInfo,
      List<String> extJarsInfo,
      List<String> allJarsInfo,
  ...
  private boolean isPackageRoot(String pkgName) {
    return rootPackages.stream().anyMatch(p -> pkgName.startsWith(p));
  }
  ...
  private Set<TypeName> computeRootClasses() throws Exception {
    ...
    for (String jarInfo : rootJarsInfo) {
      ...
      for (JarClassReader.Entry classData : jarReader) {
        ...
        if (!BY_PKG_NAME || isPackageRoot(pkgName)) {
